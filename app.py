...
with col_izq:
    ...
    with st.expander("5. Hallazgos EKG"):
        ekg_sinusal = st.checkbox("Ritmo Sinusal Normal", True)
    ...
    with st.expander("6. Datos Quirúrgicos y Plan Anestésico"):
        cx_dict = {"Colecistectomía Lap.": "Intermedio", "Apendicectomía": "Intermedio", "Hernioplastia": "Bajo", "Artroplastia": "Intermedio", "Cesárea": "Bajo", "Bypass Coronario (CRM)": "Alto", "Laparotomía": "Alto", "Otra": "Intermedio"}
        cx_sel = st.selectbox("Procedimiento:", list(cx_dict.keys()))
        nombre_cx = cx_sel if cx_sel != "Otra" else st.text_input("Nombre Cx:", "Cirugía General")
        riesgo_cx_tipo = st.selectbox("Riesgo:", ["Bajo", "Intermedio", "Alto"], index=1 if cx_sel != "Otra" else 0)
    ...
    with st.expander("7. Riesgos Adicionales (Caprini/Apfel)"):
        no_fumador = st.checkbox("NO Fumador", True)
        historia_nvpo = st.checkbox("Historia NVPO")
        opioides_post = st.checkbox("Opioides post-op", True)
...
with col_der:
    ...
    with tab1:
        if st.button("🔄 ACTUALIZAR REPORTE", type="primary"):
            st.write(f"**Paciente:** {sexo}, {edad} años. **Cx:** {nombre_cx}.")
    ...
    with tab2:
        if st.button("⚙️ GENERAR PLAN", type="primary"):
            ...
            vt = peso_predicho * 7
            st.write(f"• **Vt Protector:** {vt:.0f} mL | **PEEP:** {10 if imc>=30 else 5} cmH2O.")
            st.write(f"• **Mantenimiento:** {peso_real+40:.0f} mL/hr.")
            st.write(f"• **Sangrado Permisible:** {(peso_real * (70 if sexo == 'Masculino' else 65)) * 0.1:.0f} mL.")
